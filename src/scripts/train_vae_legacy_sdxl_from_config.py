...
def train_from_config(cfg: dict[str, Any], config_path: str | Path) -> None:
    ...
    from src.sd35_task_aware_vae.sdxl.vae_factory import build_autoencoder_kl
    ...
    if resume_dir:
        ...
        if isinstance(resume_init_report, dict):
            try:
                vae._sd35_task_aware_vae_init = resume_init_report
            except Exception:
                pass
    else:
    ...
    init_report = getattr(vae, "_sd35_task_aware_vae_init", None)
    if isinstance(init_report, dict):
        try:
            import json
